...
export class FunnelTracker extends BaseTracker {
  ...
  trackWelcomeView(): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.FUNNEL_WELCOME_VIEW, {
  ...
  trackNameEntered(nameLength: number): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.FUNNEL_NAME_ENTERED, {
  ...
  trackGenderSelected(gender: string): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.FUNNEL_GENDER_SELECTED, {
        gender,
        timestamp: this.getTimestamp(),
      });
    });
  ...
  trackJoinClicked(): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.FUNNEL_JOIN_CLICKED, {
        timestamp: this.getTimestamp(),
      });
    });
  }
  ...
  trackChatPageLoaded(): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.FUNNEL_CHAT_PAGE_LOADED, {
        timestamp: this.getTimestamp(),
      });
    });
  }
  ...
  trackSearchStarted(): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.FUNNEL_SEARCH_STARTED, {
        timestamp: this.getTimestamp(),
      });
    });
  }
  ...
  trackMatchConnected(): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.FUNNEL_MATCH_CONNECTED, {
        timestamp: this.getTimestamp(),
      });
    });
  }
  ...
  trackFirstMessageSent(): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.FUNNEL_FIRST_MESSAGE_SENT, {
        timestamp: this.getTimestamp(),
      });
    });
  }
  ...
  trackSessionCompleted(durationMs: number, messageCount: number): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.FUNNEL_SESSION_COMPLETED, {
