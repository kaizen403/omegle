...
export class MatchTracker extends BaseTracker {
  ...
  trackStart(preferences?: { camera?: boolean; microphone?: boolean; gender?: string }): void {
    ...
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.MATCH_START, {
  ...
  trackFound(waitTime?: number): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.MATCH_FOUND, {
  ...
  trackEnded(reason: MatchEndReason, duration?: number): void {
    ...
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.MATCH_ENDED, {
        reason,
      ...
      if (this.sessionStartTime) {
        ...
        this.analytics!.capture(AnalyticsEvents.SESSION_DURATION, {
  ...
  trackQuality(durationMs: number, messageCount: number): void {
    ...
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.MATCH_QUALITY_SCORE, {
        quality,
      ...
      this.analytics!.capture(AnalyticsEvents.MATCH_DURATION_BUCKET, {
        bucket,
        duration_seconds: Math.floor(durationMs / 1000),
        timestamp: this.getTimestamp(),
      });

      this.analytics!.capture(AnalyticsEvents.MESSAGES_PER_SESSION, {
        count: messageCount,
        duration_ms: durationMs,
        messages_per_minute:
  ...
  trackTimeout(waitTimeMs: number): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.SEARCH_TIMEOUT, {
