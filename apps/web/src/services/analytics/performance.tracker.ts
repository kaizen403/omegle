...
export class PerformanceTracker extends BaseTracker {
  ...
  trackPageLoadTime(loadTimeMs: number): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.PAGE_LOAD_TIME, {
  ...
  trackTimeOnPage(pageName: string, timeSeconds: number): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.TIME_ON_PAGE, {
        page_name: pageName,
  ...
  trackFirstVideoFrame(timeToFirstFrameMs: number, isLocal: boolean): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.FIRST_VIDEO_FRAME, {
  ...
  trackNetworkQuality(quality: NetworkQuality, latency?: number): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.NETWORK_QUALITY, {
        quality,
        latency_ms: latency,
        timestamp: this.getTimestamp(),
      });
  ...
  trackConnectionQualityDrop(fromQuality: string, toQuality: string): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.CONNECTION_QUALITY_DROP, {
