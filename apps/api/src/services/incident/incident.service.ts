...
export type IncidentType = 'instagram_handle' | 'phone_number' | 'email' | 'harassment' | 'spam' | 'threat' | 'pii_leak';
...
const HARASSMENT_KEYWORDS = ['kill yourself', 'kys', 'i will find you', 'i know where you live', 'send nudes', 'show me your', 'meet me alone', 'harass'];
const HARASSMENT_RE = new RegExp(HARASSMENT_KEYWORDS.map((k) => k.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|'), 'i');
...
export class IncidentService {
  ...
  async scanAndStore(opts: { roomId: string; uid: number; userName: string; text: string; timestamp: number; messageId?: string }): Promise<DetectedIncident[]> {
    ...
    try {
      const rows = hits.map((h) => ({
        ...
        snippet: opts.text.slice(Math.max(0, h.index - 24), h.index + 48).trim().slice(0, 200),
        status: 'open' as const,
        reviewedBy: null,
        createdAt: new Date(opts.timestamp),
      }));
      await db.insert(chatIncidents).values(rows);
      logger.info(`[INCIDENT] ${hits.length} incident(s) stored for room ${opts.roomId} uid ${opts.uid}: ${hits.map((h) => h.type).join(',')}`);
    } catch (error) {
  ...
  async list(filters: { roomId?: string; uid?: number; status?: string; type?: string; limit?: number; offset?: number } = {}) {
    try {
      ...
      query = query.orderBy(desc(chatIncidents.createdAt)).limit(filters.limit ?? 100).offset(filters.offset ?? 0);
  ...
  async updateStatus(id: string, status: string, reviewedBy?: string) {
    try {
      const rows = await db.update(chatIncidents).set({ status, reviewedBy: reviewedBy || null }).where(eq(chatIncidents.id, id)).returning();
      return rows[0] || null;
    } catch (error) {
  ...
  async countsByRoom(roomIds: string[]): Promise<Map<string, number>> {
    ...
    try {
      const rows = await db.select({ roomId: chatIncidents.roomId }).from(chatIncidents).where(sql`${chatIncidents.roomId} IN ${roomIds}`);
  ...
  async countsByUid(uids: number[]): Promise<Map<number, number>> {
    ...
    try {
      const rows = await db.select({ uid: chatIncidents.uid }).from(chatIncidents).where(sql`${chatIncidents.uid} IN ${uids} AND ${chatIncidents.status} = 'open'`);
