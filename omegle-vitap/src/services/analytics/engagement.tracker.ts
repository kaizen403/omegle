...
export class EngagementTracker extends BaseTracker {
  ...
  trackReturningUser(): void {
    ...
    try {
      ...
      if (isReturning) {
        ...
        this.analytics.capture(AnalyticsEvents.RETURNING_USER, {
      ...
      this.analytics.capture(AnalyticsEvents.SESSION_COUNT, {
  ...
  trackPageView(pageName: string, pageTitle?: string): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.PAGE_VIEW, {
        page_name: pageName,
  ...
  trackButtonClick(buttonName: string, buttonLocation: string): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.BUTTON_CLICK, {
  ...
  trackTabVisibility(isVisible: boolean): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.TAB_VISIBILITY_CHANGE, {
  ...
  trackEngagement(score: number, actions: string[]): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.USER_ENGAGEMENT, {
  ...
  trackUserGender(gender: string): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.USER_GENDER_SET, {
  ...
  trackGenderPreference(preference: string): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.GENDER_PREFERENCE_SET, {
  ...
  setUserProperty(key: string, value: string): void {
    if (!this.analytics) return;
    try {
      this.analytics.register({ [key]: value });
    } catch {
