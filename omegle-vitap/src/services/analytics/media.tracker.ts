...
export class MediaTracker extends BaseTracker {
  ...
  trackCameraToggle(enabled: boolean, context: MediaContext): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.CAMERA_TOGGLE, {
        enabled,
        context,
  ...
  trackMicrophoneToggle(enabled: boolean, context: MediaContext): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.MICROPHONE_TOGGLE, {
        enabled,
        context,
        timestamp: this.getTimestamp(),
      });
  ...
  trackDeviceListOpened(deviceType: DeviceType): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.DEVICE_LIST_OPENED, {
        device_type: deviceType,
        timestamp: this.getTimestamp(),
      });
    });
  ...
  trackMessageSent(messageLength: number, hasEmoji: boolean): void {
    ...
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.MESSAGE_SENT, {
  ...
  trackMessageReceived(messageLength: number): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.MESSAGE_RECEIVED, {
        message_length: messageLength,
        timestamp: this.getTimestamp(),
      });
    });
  ...
  trackEmojiUsed(): void {
    this.safeTrack(() => {
      this.analytics!.capture(AnalyticsEvents.EMOJI_USED, {
        timestamp: this.getTimestamp(),
      });
    });
  }
